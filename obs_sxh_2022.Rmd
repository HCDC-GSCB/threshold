...
```{r}
df_ewma <- ewma(df, "total", "nam", "tuan", lambda = 0.3, year_to_predict = 2022, k = 2)

df_ewma$outbreak_ewma <- ifelse(df_ewma$outbreak_ewma==0,NA,1)

df_ewma$outbreak <- ifelse(df_ewma$outbreak==0,NA,1)

filter(df_ewma, nam == 2022) %>% ggplot() +
  geom_col(aes(x = tuan, y = total)) +
  geom_line(aes(x = tuan, y = upper_bound), size = 1) +
  geom_point(aes(x = tuan, y = outbreak_ewma, color = "EWMA"), shape = 2, size = 2, position = position_nudge(y = 2)) +
  geom_point(aes(x = tuan, y = outbreak, color = "Standard"), position = position_nudge(y = -0.5)) + 
  labs(x = "Epi week", y = "Number of Cases", title = "EWMA method") +
  scale_color_manual(
    name = "Method", 
    values = c("EWMA" = "red", "Standard" = "green"))

df_ewma$outbreak_ewma <- ifelse(is.na(df_ewma$outbreak_ewma),0,1)

df_ewma$outbreak <- ifelse(is.na(df_ewma$outbreak),0,1)
```

### Phương pháp Serfling model (Linear model)

```{r}
dff <- filter(df, nam < 2022)

sm <- lm(total ~ t + sin(2*pi*t/52) + cos(2*pi*t/52), data = dff)
...
# EWMA
cfm_ewma <- filter(df_ewma, nam == 2022)
cfm_ewma$outbreak <- factor(cfm_ewma$outbreak, levels = c(0,1), labels = c(0,1))
cfm_ewma$outbreak_ewma <- factor(cfm_ewma$outbreak_ewma, levels = c(0,1), labels = c(0,1))
m17 <- caret::confusionMatrix(cfm_ewma$outbreak, cfm_ewma$outbreak_ewma)
...
ppv <- c(m1$TP/(m1$TP+m1$FP), m2$TP/(m1$TP+m1$FP), m3$TP/(m1$TP+m1$FP), 
         m4$TP/(m1$TP+m1$FP), m5$TP/(m1$TP+m1$FP), m6$TP/(m1$TP+m1$FP),
         m7$TP/(m1$TP+m1$FP), m8$TP/(m1$TP+m1$FP), m9$TP/(m1$TP+m1$FP),
         m10$TP/(m1$TP+m1$FP), m11$TP/(m1$TP+m1$FP), m12$TP/(m1$TP+m1$FP),
         m13$TP/(m1$TP+m1$FP), m14$TP/(m1$TP+m1$FP), m15$TP/(m1$TP+m1$FP),
         m16$TP/(m1$TP+m1$FP), m17$byClass["Pos Pred Value"], 
         m18$byClass["Pos Pred Value"])

npv <- c(m1$TN/(m1$TN+m1$FN), m2$TN/(m1$TN+m1$FN), m3$TN/(m1$TN+m1$FN), 
         m4$TN/(m1$TN+m1$FN), m5$TN/(m1$TN+m1$FN), m6$TN/(m1$TN+m1$FN),
         m7$TN/(m1$TN+m1$FN), m8$TN/(m1$TN+m1$FN), m9$TN/(m1$TN+m1$FN),
         m10$TN/(m1$TN+m1$FN), m11$TN/(m1$TN+m1$FN), m12$TN/(m1$TN+m1$FN),
         m13$TN/(m1$TN+m1$FN), m14$TN/(m1$TN+m1$FN), m15$TN/(m1$TN+m1$FN),
         m16$TN/(m1$TN+m1$FN), m17$byClass["Neg Pred Value"], 
